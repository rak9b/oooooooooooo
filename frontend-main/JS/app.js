...
async function handleAdminAction(actionType, payload) {
    try {
        const res = await fetch(`${API_URL}/api/admin/${actionType}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload)
        });
        const data = await res.json();
        alert(data.message || "কাজ সফল হয়েছে!");
    } catch (err) {
        console.error("বাটন কাজ করছে না কারণ:", err);
    }
}
...
async function fetchUserInfo(token) {
    try {
        const res = await fetch(`${API_URL}/api/auth/user-info`, { // ধরে নেওয়া API endpoint (ব্যাকেন্ডে অ্যাড করো যদি না থাকে)
            method: 'GET',
            headers: {
...
async function fetchAgentPayment(token) {
    try {
        const res = await fetch(`${API_URL}/api/player/deposit-info`, {
...
function createMotherPanel(name, pass) {
    callAdminAction('add-sub-user', {
        username: name,
        password: pass,
        role_to_create: 'MOTHER_PANEL'
    });
}

async function addMotherPanel() {
    const data = {
        username: "mother_test",
        password: "123",
        role_to_create: "MOTHER_PANEL"
    };
